...
class Server(object):
    def __init__(self, model, eval_dataset, num_clients):
        ...
        self.serverTestDataSet = CustomDataset(eval_dataset[0],eval_dataset[1])
...
class Client(object):
    def __init__(self, model, train_dataset,val_dataset, id = -1):
                ...
                self.client_id = id
                self.train_dataset = CustomDataset(train_dataset[0],train_dataset[1])
                self.train_loader = torch.utils.data.DataLoader(self.train_dataset, batch_size=32)
                self.eval_dataset = CustomDataset(val_dataset[0], val_dataset[1])
