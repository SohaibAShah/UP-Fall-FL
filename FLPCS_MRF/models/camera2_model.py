...
class Img2Model(nn.Module):
    def __init__(self):
        super(Img2Model, self).__init__()

        # 定义卷积层
        self.conv = nn.Conv2d(in_channels=1, out_channels=18, kernel_size=3, stride=1, padding=1)
        self.batch_norm = nn.BatchNorm2d(18)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)

        # 全连接层
        self.fc1 = nn.Linear(18 * (16) * 16, 100)
        self.dropout = nn.Dropout(0.2)
        self.fc2 = nn.Linear(100, 12)

    def forward(self, x):
        x = x.permute(0, 3, 1, 2)
        x = F.relu(self.conv(x))  # 卷积层 + ReLU
        x = self.batch_norm(x)  # 批标准化
        x = self.pool(x)  # 池化层
        x = x.view(x.size(0), -1)  # 展平
        x = F.relu(self.fc1(x))  # 第一个全连接层 + ReLU
        x = self.dropout(x)  # Dropout层
        x = F.softmax(self.fc2(x), dim=1)  # 输出层 + softmax激活
        return x
    
model_name = 'Img2Model'
def trainValImg2Model(total_client,num_clients,epoch,max_acc,
    ...
    server = Server(model_MLP, [X_test_2_scaled_splits[total_client-1],Y_test_2_splits[total_client-1]], num_clients)
    clients = []

    for c in range(total_client):
        clients.append(Client(server.global_model, [X_train_2_scaled_splits[c],Y_train_2_splits[c]],
                              [X_val_2_scaled_splits[c],Y_val_2_splits[c]], id=c))
    ...
    test_loader = torch.utils.data.DataLoader(
        CustomDataset(X_val_2_scaled_splits[total_client - 1], Y_val_2_splits[total_client - 1]), batch_size=32)
    ...
    plt.savefig(
        "./acc_lossFiles/{}_totalClient_{}_NumClient_{}_epoch_{}_time_{}.png".format(model_name, total_client, num_clients,
                                                                             epoch,
                                                                             datetime.now().strftime(
                                                                                 '%Y-%m-%d-%H-%M-%S')))
    plt.show()