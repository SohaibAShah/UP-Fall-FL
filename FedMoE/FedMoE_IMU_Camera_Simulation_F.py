...
def load_upfall_by_subject(file_path, image_path=None):
    ...
    for sub in subs:
        ...
        print(f"Subject {sub}: Loaded {len(X_all)} aligned samples.")
...
def run_custom_scenario(scenario_name, subject_data, client_configs):
    print(f"\n{'='*60}")
    ...
    # Map subjects to configs cyclically
    ...
    for i, sub_id in enumerate(subjects):
        ...
        # Cycle through configs if fewer configs than subjects
        config = client_configs[i % len(client_configs)]
        
        # Parse Config into Active Sensors List (Helper logic)
        active_sensors = []
        if 'IMU' in config:
...
if __name__ == "__main__":
    ...
    # Define the requested combinations
    configs = [
        ['IMU', 'C1', 'C2'],
        ['IMU', 'C1'],
        ['IMU', 'C2'],
    ...
    run_custom_scenario("Specific Sensor Combinations (IMU & Cameras)", data, configs)