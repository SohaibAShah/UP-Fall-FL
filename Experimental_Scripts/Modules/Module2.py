...
def load_and_prep_data(file_path):
    """Load and clean the dataset, split into train/test."""
...
class Client:
    ...
    def train(self, model, local_epochs, learning_rate):
        """Train model locally and return weights, data size, loss, and accuracy."""
...
def server_aggregate(global_model, client_updates, total_data_size):
    ...
    for k in global_dict.keys():
        global_dict[k] = torch.stack(
            [client['weights'][k].float() * client['data_size'] for client in client_updates], 0
        ).sum(0) / total_data_size
...
def run_federated_simulation(clients_data, test_loader, config, num_features):
    """Run a full FedAvg simulation and return the performance history."""
    set_seed(42)
    global_model = CNN_Attention(input_channels=num_features)
    #global_model = CNN_GAP(input_channels=num_features)
    ...
    for round_num in range(1, config['comm_rounds'] + 1):
        ...
        history['comm_mb'].append(comm_cost)
...
if __name__ == "__main__":
    ...
    # --- Run Experiments ---
    logging.info("\n--- Experiment 1: Non-IID Clients (Uniform Sampling) ---")
    ...
    logging.info("\n--- Experiment 2: IID Clients (Uniform Sampling) ---")
    ...
    logging.info("\n--- Experiment 3: Non-IID Clients (Loss-Based Sampling) ---")
