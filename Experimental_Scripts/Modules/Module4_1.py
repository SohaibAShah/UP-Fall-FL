...
def evaluate_model(model, data_loader, device, return_preds=False):
    ...
    with torch.no_grad():
        for x_csv, x_img1, x_img2, y in data_loader:
            ...
            all_probs.extend(probs[:, 1])  # Probability for class "No Fall"
...
# --- 5. Analysis and Experiment Functions ---

def correlation_analysis(motion_intensity, gate_values, output_dir):
    pearson_corr, _ = pearsonr(motion_intensity, gate_values)
    spearman_corr, _ = spearmanr(motion_intensity, gate_values)
    logging.info(f"Pearson correlation (gate vs. motion): {pearson_corr:.3f}")
    logging.info(f"Spearman correlation (gate vs. motion): {spearman_corr:.3f}")
    with open(os.path.join(output_dir, "gate_motion_correlation.txt"), "w") as f:
        f.write(f"Pearson: {pearson_corr:.3f}\nSpearman: {spearman_corr:.3f}\n")

def binned_gate_analysis(motion_intensity, gate_values, output_dir):
    import pandas as pd
    df = pd.DataFrame({'motion': motion_intensity, 'gate': gate_values})
    df['bin'] = pd.qcut(df['motion'], q=4, labels=['Low', 'Med-Low', 'Med-High', 'High'])
    stats = df.groupby('bin')['gate'].describe()
    logging.info(f"\nGate value stats by motion bin:\n{stats}")
    stats.to_csv(os.path.join(output_dir, "gate_binned_stats.csv"))

    # Boxplot
    plt.figure(figsize=(8, 6))
    df.boxplot(column='gate', by='bin')
    plt.title('Gate Value Distribution by Motion Intensity Bin')
    plt.suptitle('')
    plt.xlabel('Motion Intensity Bin')
    plt.ylabel('Gate Value')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "gate_boxplot_by_motion_bin.png"))
    plt.close()

def gate_vs_confidence_analysis(model, test_loader, device, gate_values, output_dir):
    # Run model to get prediction confidence (max softmax prob)
    model.eval()
    confidences = []
    with torch.no_grad():
        for x_csv, x_img1, x_img2, y in test_loader:
            outputs, gates = model(x_csv.to(device), x_img1.to(device), x_img2.to(device), return_gate=True)
            probs = F.softmax(outputs, dim=1)
            conf = torch.max(probs, dim=1)[0].cpu().numpy()
            confidences.extend(conf)
    # Scatter plot
    plt.figure(figsize=(8, 6))
    plt.scatter(gate_values, confidences, alpha=0.3)
    plt.xlabel('Average Gate Value')
    plt.ylabel('Prediction Confidence (max softmax)')
    plt.title('Gate Value vs. Prediction Confidence')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "gate_vs_confidence.png"))
    plt.close()

def gate_vs_error_analysis(model, test_loader, device, gate_values, output_dir):
    # Get predictions and correctness
    all_labels, all_preds, _ = evaluate_model(model, test_loader, device, return_preds=True)
    correct = (all_labels == all_preds)
    # Plot
    plt.figure(figsize=(8, 6))
    plt.hist([gate_values[correct], gate_values[~correct]], bins=30, alpha=0.7, label=['Correct', 'Incorrect'])
    plt.xlabel('Average Gate Value')
    plt.ylabel('Count')
    plt.title('Gate Value Distribution: Correct vs. Incorrect Predictions')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "gate_vs_error_hist.png"))
    plt.close()

def temporal_gate_analysis(model, test_loader, device, output_dir):
    # For demonstration, plot gate values over the first 200 samples
    gate_series = []
    with torch.no_grad():
        for x_csv, x_img1, x_img2, y in test_loader:
            _, gates = model(x_csv.to(device), x_img1.to(device), x_img2.to(device), return_gate=True)
            gate_series.extend(torch.mean(gates, dim=1).cpu().numpy())
            if len(gate_series) > 200:
                break
    plt.figure(figsize=(12, 4))
    plt.plot(gate_series[:200])
    plt.xlabel('Sample Index')
    plt.ylabel('Average Gate Value')
    plt.title('Temporal Gate Value Dynamics (First 200 Samples)')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "temporal_gate_dynamics.png"))
    plt.close()
...
if __name__ == "__main__":
    ...
    # --- Advanced Gate Analysis Experiments (for Residual Fusion) ---
    logging.info("\n--- Running Gate Analysis Experiments ---")
    ...
    correlation_analysis(motion_intensities, gate_values, OUTPUT_DIR)

    # 2. Binned Gate Analysis
    binned_gate_analysis(motion_intensities, gate_values, OUTPUT_DIR)

    # 3. Gate vs. Confidence
    gate_vs_confidence_analysis(residual_model, test_loader, config['device'], gate_values, OUTPUT_DIR)

    # 4. Gate vs. Error Cases
    gate_vs_error_analysis(residual_model, test_loader, config['device'], gate_values, OUTPUT_DIR)

    # 5. Temporal Gate Analysis
    temporal_gate_analysis(residual_model, test_loader, config['device'], OUTPUT_DIR)

    # 6. Visualize Gate Value vs. Motion Intensity (Scatter)
    ...
    # 7. Gate Adaptation Under Noisy Images
    logging.info("\n--- Gate Adaptation Under Noisy Images ---")
    gate_values_noisy = []
    with torch.no_grad():
        for x_csv_b, x_img1_b, x_img2_b, y_b in noisy_test_loader:
            _, gates = residual_model(x_csv_b.to(config['device']), x_img1_b.to(config['device']), x_img2_b.to(config['device']), return_gate=True)
            gate_values_noisy.extend(torch.mean(gates, dim=1).cpu().numpy())
    gate_values_noisy = np.array(gate_values_noisy)
    plt.figure(figsize=(10, 5))
    plt.scatter(motion_intensities, gate_values, alpha=0.3, label='Clean')
    plt.scatter(motion_intensities, gate_values_noisy, alpha=0.3, label='Noisy Images', color='red')
    plt.xlabel('Motion Intensity')
    plt.ylabel('Gate Value')
    plt.legend()
    plt.title('Gate Value vs. Motion Intensity (Clean vs. Noisy Images)')
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'gate_vs_motion_clean_vs_noisy.png'))
    plt.show()

    logging.info(f"\n✅ All simulations and analyses complete. Results saved in '{OUTPUT_DIR}'.")