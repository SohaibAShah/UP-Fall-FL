...
# =========================
# 2. DATA LOADING & CLIENT CREATION
# =========================

def load_and_prep_data(file_path):
    """Load, clean, and split the dataset into train/test."""
    ...
    cols_to_drop = [c for c in df.columns if any(x in c for x in ['Infrared', 'Tag', 'Time', 'Trial'])]
    df.drop(columns=cols_to_drop, inplace=True, errors='ignore')
    df.dropna(inplace=True)
    df.drop_duplicates(inplace=True)

    # Binary label: 0 = Fall, 1 = No Fall
    ...
    logging.info(f"Train subjects: {train_subjects}, Test subjects: {test_subjects}")
    return train_df, test_df

def create_clients(train_df, test_df):
    """Create windowed datasets for clients and a global test set."""
    ...
    for subject in test_df['Subject'].unique():
        ...
        for i in range(0, len(d) - 200, 100):
            X_test_w.append(d[i:i+200])
            y_test_w.append(0 if np.any(l[i:i+200] == 0) else 1)
    ...
    # Create Non-IID clients (by subject)
    clients = {}
    for client_id in sorted(train_df['Subject'].unique()):
        ...
        for i in range(0, len(d) - 200, 100):
            X_client_w.append(d[i:i+200])
            y_client_w.append(0 if np.any(l[i:i+200] == 0) else 1)
        X_client = np.transpose(np.array(X_client_w), (0, 2, 1))
        y_client = np.array(y_client_w)
...
class Client:
    ...
    def train(self, model, config):
        """Train the client model locally and return updates."""
        ...
        if config['algorithm'] == 'scaffold':
            # Move final weights to CPU before calculating deltas
...
def evaluate_model(model, data_loader, device):
    """Evaluate a model and return F1-score for the 'Fall' class."""
...
# =========================
# 5. FEDERATED SIMULATION RUNNER
# =========================

def run_federated_simulation(clients_data, test_loader, config, num_features):
    """Run a full FL simulation and return performance history."""
    set_seed(42)
    ...
    # SCAFFOLD: initialize server and client control variates
    ...
    if server_control_variate:
        for param in server_control_variate.parameters():
            param.data.zero_()
    ...
    for round_num in range(1, config['comm_rounds'] + 1):
        ...
        if config['algorithm'] == 'scaffold':
            for k in global_dict.keys():
                ...
                if total_data_size > 0:
                    global_dict[k] += weighted_sum_delta / total_data_size
        ...
        logging.info(f"Algorithm: {config['algorithm']:<8} | Round: {round_num:02d} | Global F1-Score: {f1:.4f}")

    # Compute worst client F1-score at the end
    worst_f1 = 1.0
    ...
    # Find rounds to reach target F1
...
if __name__ == "__main__":
    ...
    plt.ylabel('Global Test F1-Score (Fall Class)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'f1_vs_rounds.png'))
    plt.show()
