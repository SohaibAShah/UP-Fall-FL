...
from src.dataset.Dataloader import FallDetectionDataset, FallDetectionDataset_Window
from src.models.cnn1d import AdvancedCNN1D, SimpleCNN1D
...
def main(args):
    ...
    # Instantiate the new dataset for each split, passing window arguments
    ds_train = FallDetectionDataset_Window(data_dir, split='train', window_size=args.window, stride=args.stride)
    ds_val = FallDetectionDataset_Window(data_dir, split='val', window_size=args.window, stride=args.stride)
    ds_test = FallDetectionDataset_Window(data_dir, split='test', window_size=args.window, stride=args.stride)
    ...
    # --- Use the Advanced CNN Model ---
    # The dataset provides 6 sensor channels (Time column is skipped)
    model = AdvancedCNN1D(in_channels=6, num_classes=2).to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    
    best_f1, best_model_state = 0.0, None

    print("--- Starting Training with 1D CNN ---")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Training script for 1D CNN fall detection.")
    parser.add_argument("--epochs", type=int, default=30, help="Number of training epochs.")
    parser.add_argument("--bs", type=int, default=64, help="Batch size.")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate.")
    # Add arguments for windowing to make the script flexible
    parser.add_argument("--window", type=int, default=150, help="Window size for time-series.")
    parser.add_argument("--stride", type=int, default=75, help="Stride for sliding window.")
