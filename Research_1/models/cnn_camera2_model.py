...
def build_cnn_camera2_model(input_shape):
    """
    Builds and compiles a CNN model for Camera 2 image data.
...
def train_and_evaluate_cnn_camera2(X_train, Y_train, X_val, Y_val, X_test, Y_test, y_test_raw):
    """
    Trains, evaluates, and saves the CNN model for Camera 2.
    ...
    """
    print("\n--- Training CNN Model for Camera 2 Images ---")
    model_img2 = build_cnn_camera2_model(X_train.shape[1:])
    model_img2.summary()

    # Create directory for saving models if it doesn't exist
    os.makedirs(os.path.dirname(config.CNN_IMG2_MODEL_PATH), exist_ok=True)

    f1_callback2 = ModelCheckpoint(
        config.CNN_IMG2_MODEL_PATH,
    ...
    print("\n--- CNN Camera 2 Model Evaluation ---")
    print("best model: ")
    model_img2.load_weights(config.CNN_IMG2_MODEL_PATH)

    print('Validation Set')
    val_results = model_img2.evaluate(X_val, Y_val, verbose=0)
    ...
    print('Test Set')
    test_results = model_img2.evaluate(X_test, Y_test, verbose=0)
    ...
    y_pred_img2_prob = model_img2.predict(X_test)
    y_pred_img2_labels = tf.argmax(y_pred_img2_prob, axis=1).numpy()
    display_result(y_test_raw, y_pred_img2_labels)

    plot_training_history(history_img2, "CNN (Camera 2 Images)")